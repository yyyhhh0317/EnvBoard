...
export const BUILTIN_TEMPLATES: ConfigTemplate[] = [
  // ===== 通用 =====
  {
    ...
    category: 'general',
    variables: [
      { key: 'NODE_ENV', placeholder: 'development', comment: '运行环境：development / production / test', isSensitive: false, expectedType: 'string', required: true, enum: ['development', 'test', 'staging', 'production'] },
      { key: 'APP_NAME', placeholder: 'my-app', comment: '应用名称', isSensitive: false, expectedType: 'string' },
      { key: 'APP_PORT', placeholder: '3000', comment: '服务监听端口', isSensitive: false, expectedType: 'number', required: true, pattern: '^\\d{1,5}$' },
      { key: 'APP_HOST', placeholder: '0.0.0.0', comment: '服务监听地址', isSensitive: false, expectedType: 'string' },
      { key: 'LOG_LEVEL', placeholder: 'info', comment: '日志级别：debug / info / warn / error', isSensitive: false, expectedType: 'string', enum: ['debug', 'info', 'warn', 'error'] },
